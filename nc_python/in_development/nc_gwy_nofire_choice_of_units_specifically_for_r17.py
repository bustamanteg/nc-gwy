...
# Code that converts scans in the nc format, produced by gxsm to gwy format. 
#created by Logan FairgrieVe-Park, modified by Jose Bustamante. 

# We will save one channel with the average dissipation, another channel using the dissipation far away from the sample. 

# The conversion factor for the frequency shift is 2mV/Hz. 
...
def run(direct,prefix, cantsens, signalgain, Q, f0, ex_units="eV"):
    ...
    w0 = f0*2*np.pi
    Adrive0=5.1 # V, from the comments of the scan it  is the dissipation at 0V, far away. 
    ...
    #Original Driving amplitudes 
    Adrive_r = datapull(fileprefix+'-Xp-ADC2.nc',1)
    print('average driving amplitude right',botqrtavg(Adrive_r))
    Adrive_l = datapull(fileprefix+'-Xm-ADC2.nc',1)
    print('average driving amplitude left', botqrtavg(Adrive_l))
    
    #Saving a separate channel to save excitation in V     
    Adrive_r=Adrive_r
    Adrive_l=Adrive_l
    print(' ')
    print('Average value of driving amplitude in V',botqrtavg((Adrive_r+Adrive_l)/2) )
    ...
    if ex_units=="Hz":
        ...
        Diss_r = (Adrive_r/botqrtavg(Adrive_r)-1)*w0/Q

        Diss_l = (Adrive_l/botqrtavg(Diss_l)-1)*w0/Q

        Diss_r0 = (Adrive_r/Adrive0-1)*w0/Q

        Diss_l0 = (Adrive_l/Adrive0-1)*w0/Q
...
Springer, hardcover ed., 7 2002.
        ...
        print('cantilever amplitude from scan',A_from_scan) 
        ...
        Diss_r = (Adrive_r/botqrtavg(Adrive_r)-1)*E0_eV

        Diss_l = (Adrive_l/botqrtavg(Adrive_l)-1)*E0_eV

        # Calculate the dissipation with respect to the dissipation far away

        # This is the dissipation as defined in the ncafm book
        Diss_r0 = (Adrive_r/Adrive0-1)*E0_eV

        Diss_l0 = (Adrive_l/Adrive0-1)*E0_eV
...
# Ifs for dissipation units
    ...
    obj['/12/data'] = GwyDataField(Adrive_r, xreal=xrange, yreal=yrange, xoff=xoff, yoff=yoff, si_unit_xy='m',si_unit_z='V')
    obj['/13/data/title'] = 'Excitation amplitude <-'
    obj['/13/data'] = GwyDataField(Adrive_l, xreal=xrange, yreal=yrange, xoff=xoff, yoff=yoff, si_unit_xy='m',si_unit_z='V')
    ...
    obj.tofile(direct+'converted_to_gwy/'+prefix+ex_units+'.gwy')
...
Diss_r0,Adrive_r,Adrive0=run(direct,prefix, cantsens, signalgain, Q, f0,"eV")
